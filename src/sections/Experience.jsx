...
const experiences = [
    {
        ...
        index: "01",
        location: "Pune, India",
        tech: ["Angular", "TypeScript", "JavaScript", "React"],
        highlight: "First production deployment",
        description:
            "Gained hands-on exposure across the full web development stack, collaborating with senior engineers on real client projects and shipping production features.",
        image: "https://careers.persistent.com/images/site/persistent-careers-opportunities.jpg",
    ...
    },
    {
        ...
        index: "03",
        location: "Remote",
        tech: ["Next.js", "TypeScript", "Tailwind", "Angular"],
        highlight: "3 product launches",
        description:
            "Engineered the frontend of 3 custom product designs using Next.js and TypeScript, implementing scalable data binding patterns to meet high-demand business requirements. Made real time client review experience.",
        image: "https://content.jdmagicbox.com/comp/kolkata/d6/033pxx33.xx33.220211091125.v9d6/catalogue/sentientgeeks-software-and-consultancy-pvt-ltd-salt-lake-city-sector-5-kolkata-computer-software-developers-ol4pdot2b7.jpg",
...
// ── Ambient background: quiet grid + drifting glow orbs ──
// Fills the dead space around the timeline without competing with it.
    ...
    return (
        <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none', zIndex: 0 }}>
            <div style={{
                ...
                maskImage: 'radial-gradient(ellipse 75% 65% at 50% 38%, #000 35%, transparent 88%)',
                WebkitMaskImage: 'radial-gradient(ellipse 75% 65% at 50% 38%, #000 35%, transparent 88%)',
            }} />
            <div className="exp-orb-a" style={{
                position: 'absolute', top: '-12%', right: '-6%',
                width: '420px', height: '420px', borderRadius: '50%',
                background: 'radial-gradient(circle, rgba(28,216,210,0.16), transparent 70%)',
                filter: 'blur(20px)',
            }} />
            <div className="exp-orb-b" style={{
            ...
            <style>{`
                @keyframes expOrbFloatA { 0%,100% { transform: translate(0,0); } 50% { transform: translate(-18px, 22px); } }
                @keyframes expOrbFloatB { 0%,100% { transform: translate(0,0); } 50% { transform: translate(20px, -16px); } }
                .exp-orb-a { animation: expOrbFloatA 16s ease-in-out infinite; }
                .exp-orb-b { animation: expOrbFloatB 19s ease-in-out infinite; }
            `}</style>
        </div>
    );
}

// ── Career snapshot strip: real numbers derived from `experiences` ──
// Sits in the gap between the divider and the timeline on both layouts.
function StatsStrip({ experiences, isMobile }) {
    const companies = new Set(experiences.map(e => e.company)).size;
    const techCount = new Set(experiences.flatMap(e => e.tech)).size;
    const years = experiences.flatMap(e => (e.duration.match(/\d{4}/g) || []).map(Number));
    const startYear = Math.min(...years);
    const totalYears = Math.max(1, new Date().getFullYear() - startYear);

    const stats = [
        { icon: <Calendar size={13} color="#a855f7" />, value: `${totalYears}+`, label: 'Years' },
        { icon: <Building2 size={13} color="#ec4899" />, value: companies, label: companies === 1 ? 'Company' : 'Companies' },
        { icon: <Code2 size={13} color="#1cd8d2" />, value: `${techCount}+`, label: 'Technologies' },
        { icon: <Briefcase size={13} color="#1cd8d2" />, value: experiences.length, label: 'Roles' },
    ];
    ...
    return (
        <div style={{
            ...
            flexWrap: 'wrap',
            padding: isMobile ? '18px 24px 0' : '22px 80px 0',
            flexShrink: 0,
        }}>
            {stats.map((s, i) => (
                <div key={s.label} style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '14px' : '28px' }}>
                    {i > 0 && <span style={{ width: '1px', height: '14px', background: 'rgba(255,255,255,0.12)' }} />}
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        {s.icon}
                        <span style={{ fontFamily: 'monospace', fontSize: isMobile ? '12px' : '13px', color: '#fff', fontWeight: 600 }}>
                            {s.value}
                        </span>
                        <span style={{ fontFamily: 'monospace', fontSize: isMobile ? '10px' : '11px', color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: '1px' }}>
                            {s.label}
                        </span>
                    </div>
                </div>
...
// ── One-line career caption — fills the gap between the stats row and the timeline ──
function CareerTagline({ experiences, isMobile }) {
    const first = experiences[0];
    const last = experiences[experiences.length - 1];
    const startYear = (first.duration.match(/\d{4}/) || [''])[0];

    return (
        <p style={{
            margin: 0,
            padding: isMobile ? '12px 24px 0' : '14px 80px 0',
            ...
            From {first.role} to {last.role} — building production web products since {startYear}.
        </p>
    );
}

// ── Vertical scroll axis — a real line, ticked off by year, with a marker ──
// that travels as the section is scrolled. Fills the left margin's empty
// height on both layouts instead of leaving it blank.
function ScrollAxis({ scrollYProgress, experiences, triggers, isMobile }) {
    const rawHeight = useTransform(scrollYProgress, [0, 1], ['0%', '100%']);
    const fillHeight = useSpring(rawHeight, { stiffness: 80, damping: 22 });

    return (
        <div style={{
            position: 'absolute',
            left: isMobile ? '14px' : '44px',
            top: isMobile ? '188px' : '212px',
            ...
            <motion.div style={{
                ...
                height: fillHeight,
                background: 'linear-gradient(180deg, #1cd8d2, rgba(168,85,247,0.6))',
            }} />
                ...
                return (
                    <div key={i} style={{
                        ...
                        <span style={{
                            ...
                            background: '#1cd8d2', transform: 'translateX(-2px)', flexShrink: 0,
                        }} />
                        {!isMobile && (
                            <span style={{
            ...
            <motion.div style={{
                position: 'absolute', left: 0, top: fillHeight,
                transform: 'translate(-50%, -50%)',
                width: '9px', height: '9px', borderRadius: '50%',
                background: '#fff',
                boxShadow: '0 0 14px rgba(255,255,255,0.7), 0 0 0 4px rgba(28,216,210,0.18)',
            }} />
        </div>
    );
}
            ...
            <div ref={sceneRef} style={{ height: `${SCENE_HEIGHT_VH}vh`, minHeight: '120vh' }}>
                <div style={{ position: 'sticky', top: 0, height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                    ...
                    <div style={{ position: 'relative', zIndex: 1, height: '100%', display: 'flex', flexDirection: 'column' }}>
                        ...
                        <StatsStrip experiences={experiences} isMobile={isMobile} />
                        <CareerTagline experiences={experiences} isMobile={isMobile} />
                        <ScrollAxis
                            scrollYProgress={scrollYProgress}
